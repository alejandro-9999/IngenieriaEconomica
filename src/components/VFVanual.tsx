...
function VFVanual() {
    ...
    const formula = `VP = A \\cdot \\left[  \\frac{(1 + i)^n - 1}{i} \\right]`;
    ...
    const calculate = () => {
        ...
        const A = (principal * (Math.pow(1 + decimalInterestRate, compoundingPeriods) - 1)) / decimalInterestRate;
    ...
    useEffect(() => {
        ...
        const new_formule = `${fomuleValidator(result,'VP')} = ${fomuleValidator(principal,'A')} \\cdot \\left[  \\frac{(1 + ${fomuleValidator(interestRate/100,'i')})^{${fomuleValidator(compoundingPeriods,'n')}} - 1}{${fomuleValidator(interestRate/100,'i')}} \\right]`;
