...
function VPAanual() {
    ...
    const formula = `VP = A \\cdot \\left[  \\frac{1 - (1 + i)^{-n}}{i} \\right]\\left( 1 + i \\right)`;
    ...
    useEffect(() => {
        ...
        const new_formule = `${fomuleValidator(result,'VP')} = ${fomuleValidator(principal,'A')} \\cdot \\left[  \\frac{1-(1 + ${fomuleValidator(interestRate/100,'i')})^{-${fomuleValidator(compoundingPeriods,'n')}}}{${fomuleValidator(interestRate/100,'i')}} \\right]\\left( 1 + ${fomuleValidator(interestRate/100,'i')} \\right)`;
